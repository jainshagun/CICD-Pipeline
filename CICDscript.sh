...
curl -u admin:admin -X GET -H "Content-Type:application/json" "http://localhost:8081/rest/agile/1.0/board/1/issue?fields=summary&jql=project=$key+AND+status!=Closed+AND+summary~%22$STR%22+ORDER+BY+key+DESC" > "/tmp/splunk/input_solr.json"
...
if [ "$findSummary" != "null"  ]
        then
                        ...
                        var=`echo curl -D- -u admin:admin -X PUT --data "$commentData" -H "$contentType" "$url"`
                        eval $var
else
                echo "Creating ticket under CICD Board"
                url="http://localhost:8081/rest/api/2/issue/"
				summaryData="'"{"\"fields\"":{"\"project\"":{"\"key\"":"\"$key\""},"\"summary\"":"\"$sum\"","\"description\"":"\"$des\"","\"issuetype\"":{"\"name\"":"\"Request\""}}}"'"
...
if [ -z $jira ] || [ $jira = $empty ]
        ...
        STR2=$jira
        STR3=`echo ${STR2// /%20}`
        curl -u admin:admin -X GET -H "Content-Type:application/json" "http://localhost:8081/rest/agile/1.0/board/715/issue?fields=summary&jql=project=$key+ORDER+BY+key+DESC" > "/tmp/splunk/input_cicd2.json"
                ...
                then
                        ...
                        while [ $i -lt $totaljira ]
                                do
                                        ...
                                        eval curl -D- -u admin:admin -X POST --data "$linksummaryData" -H "$contentType" "$linkurl"
                                        i=$((i+1))
                        done
        fi
fi
